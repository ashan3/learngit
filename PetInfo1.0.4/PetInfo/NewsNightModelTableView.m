...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        if (_imageData.count>0&&indexPath.section==0) {
            ...
            if (imageCell == nil) {
                ...
                self.label.text = [_imageData[0] objectForKey:@"RESOURCETITLE"];
...
- (UIView *)pageAtIndex:(NSInteger)index
{
    ...
    NSURL  *url = [NSURL URLWithString:[_imageData[index] objectForKey:@"COVERIMAGEURL"] ];
    if([[DataCenter  getConnectionAvailable] isEqualToString:@"none"]){
        NSString *filename = [[[_imageData[index] objectForKey:@"COVERIMAGEURL"] componentsSeparatedByString:@"/"] lastObject];
