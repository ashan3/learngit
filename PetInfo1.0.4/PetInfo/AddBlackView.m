...
@interface AddBlackView ()

@end

@implementation AddBlackView
...
- (BOOL) refresh
{
  if (![super refresh])
    return NO;
  curPage=1;
  [self performSelector:@selector(getBlackUsersList) withObject:nil afterDelay:1.0];
...
-(void)getBlackUsersList{
  NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
  NSString *userId = [[userDefaults objectForKey:@"userinfo"] objectForKey:@"USERID"];
  NSString *Url = [NSString stringWithFormat:@"UserInfoService/getTaxBlackUsers/%@/%d/%d",userId,curPage,KPageSize];
...
-(void)sender:(id)sender{
  ...
  NSString *Url = [NSString stringWithFormat:@"UserInfoService/updateTaxBlack/%@/0",str];
  [DataService requestWithURL:Url andparams:nil andhttpMethod:@"GET" completeBlock:^(id result) {
    NSString *result1 = [result objectForKey:@"result"];
    NSLog(@"result = %@",result);
    if ([result1 isEqualToString:@"0"]) {
